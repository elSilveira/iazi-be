...
import { 
  ...
  transformService, 
  transformCompany,
  buildServicesByProfessional,
  buildServicesWithProfessionals,
  isUUID 
  ...
  asyncHandler(async (req: Request, res) => {
    ...
    try {
      ...
      if (fetchServices) {
        ...
        console.log('Raw service data structure (first service):', 
          JSON.stringify(servicesData[0], null, 2));
        ...
        services = servicesData.map(service => {
          ...
          if (professionalId && Array.isArray(transformed.professionals)) {
            transformed.professionals = transformed.professionals.filter((p: any) => p.id === professionalId);
          }
          return transformed;
        });
        
        // Add debug logging to help diagnose data structure issues
        console.log('Sample professional services data:', 
          professionals.length > 0 ? 
            `First professional has ${professionals[0].services?.length || 0} services` : 
            'No professionals found');

        console.log('Sample service professionals data:', 
          services.length > 0 ? 
            `First service has ${services[0].professionals?.length || 0} professionals` : 
            'No services found');
          ...
          if (service.professionals && Array.isArray(service.professionals)) {
            service.professionals.forEach((prof: any) => {
              ...
              if (prof.services.length === 0) {
                ...
                const servicesForThisProf: any[] = [];
                
                // Search in all services for this professional's services
                services.forEach((otherService: any) => {
                  if (otherService.professionals) {
                    const matchingProf = otherService.professionals.find((p: any) => p.id === prof.id);
                    if (matchingProf && matchingProf.services && matchingProf.services.length > 0) {
                      // Add services that don't already exist in the array
                      matchingProf.services.forEach((svc: any) => {
                        if (!servicesForThisProf.some((s: any) => s.id === svc.id)) {
      ...
      if (services.length > 0) {
        ...
        professional_services = buildServicesWithProfessionals(services);
        
        // Also prepare professionals with services from the services data
        // This ensures consistency between both representations
        professionalServices = buildServicesByProfessional(services);
      ...
      // If we don't have services data, but have professionals data
        ...
        const servicesFromProfessionals: any[] = [];
        ...
        professional_services = buildServicesWithProfessionals(servicesFromProfessionals);
      }
